    ...
    <header className="bg-paper text-ink w-full">
      ...
      <div className="flex flex-row items-center justify-between py-2 px-4 md:px-8 prose-nyt text-xs md:text-sm uppercase font-sans">
        ...
        <nav className="flex-1 flex justify-center gap-6">
          {NAV_LINKS.map(link => (
            <a
              key={link.name}
              href={link.href}
              className="font-sans text-xs md:text-sm uppercase tracking-widest hover:underline"
            >
              {link.name}
            </a>
          ))}
        ...
        <div className="flex justify-end">
          <button
            className="border border-ink rounded px-4 py-1 text-ink bg-transparent font-sans text-xs md:text-sm uppercase tracking-widest hover:bg-ink hover:text-paper transition"
            type="button"
          >
            Subscribe
          </button>
